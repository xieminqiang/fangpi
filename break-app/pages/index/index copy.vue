              ...
              <scroll-view class="switch-scroll-view" scroll-x>
                <view class="switch-wrapper">
                  <button 
                    v-for="(volume, index) in volumeLevels" 
                    :key="index"
                    :class="['switch-option', { 'selected': makeupVolume === volume.value }]"
                    @click="makeupVolume = volume.value"
                  >
                    <image :class="['switch-emoji-img', volume.sizeClass]" :src="volume.icon" mode="aspectFit"></image>
                    <text class="switch-emoji-text">{{ volume.label }}</text>
                  </button>
...
const confirmMakeup = async () => {
  ...
  try {
    ...
    const requestData = {
      ...
      volumeLevel: volumeMap[makeupVolume.value],
...
const confirmFart = async () => {
  ...
  try {
    ...
    const fartDate = `${year}-${month}-${day}` // 本地日期
...
.switch-option.selected .switch-emoji-text {
  color: #141b0d;
  font-weight: 600;
}
