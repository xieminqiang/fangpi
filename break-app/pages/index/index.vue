   ...
   <!-- <view class="header">
    ...
    <view class="stats-card">
      ...
      <view class="stats-grid">
        ...
        <view class="stat-item">
          ...
          <text class="stat-value">{{ mostHappyMood }}</text>
        </view>
        <view class="stat-item">
    ...
    <view class="main-button-container">
      <button class="main-button" @click="openPopup">
        <text class="button-text">放了</text>
      </button>
    </view>
    ...
    <uni-popup ref="popup" type="bottom" :safe-area="false">
      <view class="popup-content">
            ...
            <view class="switch-wrapper">
              ...
              >
                <text class="switch-text">
                  <text class="switch-emoji">{{ type.emoji }}</text>
        ...
        <view class="section">
          ...
          <view class="switch-container">
            <view class="switch-wrapper">
              ...
              >
                <text class="switch-text">
                  <text class="switch-emoji">{{ level.emoji }}</text>
        ...
        <view class="section">
          ...
          <view class="switch-container">
            <view class="switch-wrapper">
              ...
              >
                <text class="switch-text">
                  <text class="switch-emoji">{{ mood.emoji }}</text>
        ...
        <button class="confirm-button" :disabled="isSubmitting" @click="confirmFart">
          <text class="confirm-text">{{ isSubmitting ? '提交中...' : '确认打卡' }}</text>
        </button>
      </view>
    </uni-popup>
    ...
    <uni-popup ref="makeupPopup" type="bottom" :safe-area="false">
      <view class="popup-content makeup-popup-content">
        ...
        <scroll-view class="popup-scroll-content" scroll-y>
          <!-- 弹窗标题 -->
          <view class="popup-header">
            <text class="popup-title">今日补卡</text>
          </view>
        ...
        <view class="popup-footer-fixed">
          <button class="confirm-button" :disabled="isSubmittingMakeup" @click="confirmMakeup">
            <text class="confirm-text">{{ isSubmittingMakeup ? '提交中...' : '确认补卡' }}</text>
          </button>
        </view>
      </view>
    </uni-popup>
    ...
    <uni-popup ref="audioSettingPopup" type="bottom" :safe-area="false" class="audio-setting-popup">
      <view class="popup-content audio-setting-popup-wrapper">
        ...
        >
          ...
          <view 
            id="audio-default"
            class="audio-item-default" 
            :class="{ 'selected': selectedAudioUrl === defaultAudioUrl }"
            @click="selectAudio(defaultAudioUrl, '默认音频')"
          >
            <view class="audio-item-left">
              <text class="audio-item-emoji">🔊</text>
              <view class="audio-item-info">
                <text class="audio-item-name">默认音频</text>
                <text class="audio-item-desc">系统默认放屁音效</text>
              </view>
            </view>
            <image 
              v-if="selectedAudioUrl === defaultAudioUrl" 
              class="audio-item-check" 
          ...
          <view 
            ...
            @click="selectAudio(audio.url, audio.name)"
          >
            <!-- 选中指示器（右上角） -->
            <image 
        ...
        <view class="popup-footer popup-footer-fixed">
          <button class="cancel-btn" @click="closeAudioSettingPopup">取消</button>
          <button class="confirm-btn" @click="saveAudioSetting">确认</button>
        </view>
      </view>
    </uni-popup>
...
const confirmFart = async () => {
  ...
  try {
    ...
    // 获取当前时间
    const now = new Date()
    const fartDate = now.toISOString().split('T')[0] // 2025-10-17
